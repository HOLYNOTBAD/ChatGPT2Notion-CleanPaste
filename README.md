...
## 📝 修复笔记（2026-03-03）
### 问题现象
- 从 ChatGPT 复制到 Notion 时，多级列表等 Markdown 结构被抹平。

### 根因分析
- 复制时 `text/html` 被重写为“纯文本 + `<br>`”。
- 结构化标签（如 `ul/ol/li`）丢失，导致 Notion 只能按普通段落粘贴。

### 修复内容
- 改为从选区克隆后的 DOM 直接生成 `text/html`，保留列表与层级结构。
- `text/plain` 仍保持清洗逻辑（数学节点去重与 LaTeX 归一化）。
